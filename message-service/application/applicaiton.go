...
func SetupRouter(h *handler.Handler) *chi.Mux {
	...
	router.Get("/api/v1/messages/ws", h.HandleMessage)
	// router.Get("/api/v1/messages", h.GetAllMessages)
	router.Get("/api/v1/messages/users/{userId}", h.GetAllUserMessages)
	router.Get("/api/v1/messages/page", h.GetUserMessages)
	router.Get("/api/v1/users/{userId}/checkOnline", h.CheckOnline)
	return router
}
