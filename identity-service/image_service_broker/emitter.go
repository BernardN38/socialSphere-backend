...
func (e *Emitter) Push(event []byte, queue string, key string, contentType string) error {
	channel, err := e.connection.Channel()
	if err != nil {
		return err
	}
	err = channel.PublishWithContext(context.Background(), "image-service", queue, false, false, amqp.Publishing{
		DeliveryMode: amqp.Persistent, ContentType: "multipart", Body: event, Headers: map[string]interface{}{"imageId": key, "contentType": contentType},
	})
	if err != nil {
		return err
	}
